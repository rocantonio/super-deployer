...
app.ws('/main', (ws, req) => {
  ...
  ws.on('message', message => {
    ...
    for (var i in aProjects) {
      ...
      if (oProj.mtad) {
        project = project + '&& cd ' + aProjects[i].uri.fsPath.substr(0, aProjects[i].uri.fsPath.length - 9) + ` && cds build/all --clean && mbt assemble && ${cfVersion} deploy mta_archives/` + aProjects[i].mtad.ID + '_' + aProjects[i].mtad.version + '.mtar ';
      } else {
        project = project + '&& cd ' + aProjects[i].uri.fsPath.substr(0, aProjects[i].uri.fsPath.length - 8) + ` && mbt build && ${cfVersion} deploy mta_archives/` + aProjects[i].mtad.ID + '_' + aProjects[i].mtad.version + '.mtar ';
      }
